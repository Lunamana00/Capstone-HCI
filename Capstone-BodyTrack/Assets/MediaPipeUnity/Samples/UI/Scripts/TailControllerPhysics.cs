...
public class TailControllerPhysics : MonoBehaviour, ILimbPhysicsData
{
    ...
    void Start()
    {
        ...
        if (imuProvider == null)
        {
            Debug.LogWarning("IMU input provider not found. Tail will use zero input.");
        }
    ...
    void Update()
    {
        if (allowKeyboardToggle && Input.GetKeyDown(toggleInputKey))
        {
            ...
            Debug.Log($"Tail input source: {(simulateInput ? "Keyboard" : "IMU")}");
        }
...
=======
        ...
        float pitch = currentAccel.z * forceMagnitude * forceScale; // Forward/Back tilt
        float yaw = currentAccel.x * forceMagnitude * forceScale;   // Left/Right tilt
...
>>>>>>> Stashed changes
    ...
    {
        get { return tailRoot != null ? tailRoot : transform; }
    }
